...
from utils.datasets import TrainDataset, ISIC2020
...
def train_isic2020():
    ...
    isic2020_data = ISIC2020(base_dir=train_args.train_path, labels_path=train_args.labels_path)

    # get timm model
    bb_model, transform, inpt_dim = get_tim_model(m_name=train_args.m_name, 
                                                pretrained=True, 
    ...
    params = {'batch_size': train_args.batch_size,
            'shuffle': True,
            'num_workers': train_args.num_workers}

    data_loader = torch.utils.data.DataLoader(
                        TrainDataset(file_paths = isic2020_data.image_files, 
    ...
    for epoch in range(train_args.n_epoch):
        ...
        running_loss = 0.0
        n_mini_batch = math.ceil(len(isic2020_data.image_files) / train_args.batch_size)
    ...
    m_params = sum([torch.tensor(param.shape).prod() for param in bb_model.parameters()])
